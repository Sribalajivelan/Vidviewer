...
// next to the original file. Returns a job id to poll via getJob().
async function startConversion(source, relPath) {
  ...
  if (fs.existsSync(outputPath)) {
    ...
    jobs.set(jobId, { status: 'done', percent: 100, relOutputPath: outRelPath });
  ...
  const jobId = crypto.randomUUID();
  const job = { status: 'running', percent: 0, relOutputPath: outRelPath, error: null };
  jobs.set(jobId, job);
...
module.exports = { startConversion, getJob };
