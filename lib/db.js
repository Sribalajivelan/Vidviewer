...
const { nameFromUrl } = require('./urlSource');
...
const playback = {
  ...
  listRecent(limit = 20) {
    const rows = db
      .prepare(
        `SELECT p.id AS id, p.source_id AS sourceId, s.name AS sourceName,
                s.type AS sourceType, s.url AS sourceUrl,
    ...
    // The player needs a mime to know how to load the file. For local/FTP
    // rows `path` is a real filename with an extension; a "url" source's
    // path is always "" (it's a single-file source), so its mime has to
    // come from the underlying URL instead. Every row here is a video -
    // saveProgress is only ever called for video playback.
    return rows.map(({ sourceType, sourceUrl, ...row }) => ({
      ...row,
      mime: (sourceType === 'url' ? mimeFor(nameFromUrl(sourceUrl)) : mimeFor(row.path)) || 'video/mp4',
    }));
  },
